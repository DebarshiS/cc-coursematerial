# Basic Network Knowhow for Cloud App Developers

When you work in cloud applications development and testing you will occasionally have issues related to 
networking, e.g. that a certain computer / server cannot be reached, where and why you need to set proxies etc. 
This document describes some basic networking knowhow that may come in handy when you need to troubleshoot these situations.

We first describe the practical 'troubleshooting' tips afterwards - for those who want to know more - some networking basics that may be helpful.

# Typical Problems and Resolutions
* **Unknown Host when running a service locally that needs to connect to services on CF:** When you test your service locally and your service needs to connect to another service that runs on CF, you will usually get something like 'host https://bulletinboard-users-course.cfapps.sap.hana.ondemand.com unknown'. To solve this, specify as VM parameters in your launch config ` -Dhttp.proxyHost=proxy.wdf.sap.corp -Dhttp.proxyPort=8080 -Dhttp.nonProxyHosts=localhost|127.*|[::1]|*.sap.corp `. Note the blanks at the beginning and in between the -D parameters!
* 
...
* Networks are generally described in layers.
...
* The [OSI Stack model](https://en.wikipedia.org/wiki/OSI_model) is pretty much the standard for networking. Levels 1 - 4 are well standardized and will be described briefly below. The higher layers are not relevant here.  
...
Business networks are divided into zones according to the type of systems and the security requirements / access restrictions. The picture below shows the network zones of SAP
![SAP Network Design](images/SAP_NetworkDesign.png)

Notes:
* Zone 1 is the most secure zone, zone 5 is the extranet / internet. 
* Access from higher to lower zones is restricted by firewalls between the zones that restrict network traffic according to 
  - restricting certain machines / users 
  - only allowing certain protocols
  - only allowing certain operations
  - usually a combination of these with black list, white list specifications. 
* When a company has web servers / applications, they are usually put into a DMZ (= demilitarized zone), in this case zone 4. All publicly visible SAP systems accessible via internet are placed here. Then such a system needs access to a 'backend system', it will use e.g. another protocol such as RFC on a specific port that is explicitly opened for that one client system. 
* You can find the details of SAP network design (current version) on [this portal page](•	https://portal.wdf.sap.corp/irj/portal?NavigationTarget=navurl://008bd94c118ea83047682b502dc2336e), then click on 'global security directives' and then on 'network design'.

* **sap.hana.ondemand.com** is in Zone 4 because it is the publicly hosted SAP CP platform
* **cfapps.sap.hana.ondemand.com** is the CF landscape that appears as a part of the public SAP CP but is actually internal since it is not visible from outside SAP yet. 
 

# Links and Resources
* [SAP Internet Information](https://internet.sap.corp/) 
