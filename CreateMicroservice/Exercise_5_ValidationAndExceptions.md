...
- Extend the `AdvertisementControllerTest` class and add a test which does a GET request using the ID "-1". Your expectation is that the server responds with a HTTP status code `400` (Bad Request).
...
- To implement a validation of the ID path parameter you need to add the `@Min(0)` annotation to your ID argument in the `AdvertisementController` class. This indicates that only non-negative IDs are valid for this method. And the class need to be annotated with `@Validated` annotation, for their methods to be searched for inline constraint annotations. 
...
- To fix the failing test, we want to implement an exception handler which automatically transforms exceptions of type `ConstraintViolationException` to respond with a HTTP code 400 (Bad Request). This response should also include a meaningful error message.

- Create a class `CustomExceptionMapper` in the package `com.sap.bulletinboard.ads.controllers` and copy the code from [here](https://github.wdf.sap.corp/raw/cc-java/cc-bulletinboard-ads-spring-webmvc/solution-5-ValidationExceptions/src/main/java/com/sap/bulletinboard/ads/controllers/CustomExceptionMapper.java).
...
**Note:** The structure of the error response does not yet fully implement the REST API Standard, which are rolled out via the [CTO Whitepaper](https://jam4.sapjam.com/wiki/show/eBIJTH4EwfD15ymE2nv2pG) as 
...
- Write a test creating a new advertisement with `title` set to `null` or even "". Your expectation is that the server responds with a HTTP status code `400` (Bad Request).
...
## Further Reading
- [REST API Harmonization Direction (CTO Whitepaper)](https://jam4.sapjam.com/wiki/show/eBIJTH4EwfD15ymE2nv2pG)
- [Java Bean Validation - API JavaDoc](https://docs.oracle.com/javaee/7/api/index.html?javax/validation/constraints/package-summary.html)
- [Hibernate Validation - API JavaDoc](https://docs.jboss.org/hibernate/validator/6.0/api/)
- [JSR-303 Bean Validation - Specification](http://download.oracle.com/otndocs/jcp/bean_validation-1.0-fr-oth-JSpec/)	
- [HTTP Status Codes](http://www.restapitutorial.com/httpstatuscodes.html)
- [Exception Handler (Example)](https://www.jayway.com/2013/02/03/improve-your-spring-rest-api-part-iii/)
- [Spring(boot) tutorial - validation](https://github.wdf.sap.corp/d022051/SpringTutorial/wiki/Validation)
